...
#makde the data to where it only contians: zip_code, median income, income class, IMR, pred_white, hispanic, white, black
```

##create binary data, which will be a 1 if a zipcode is predominantly white (proportion of 0.5 or greater) or not predominantly white
```{r}
zipcode <- zipcode %>% mutate(pred_white = case_when(white >= 0.5 ~ 1,
                                           white < 0.5 ~ 0))
```
...
```{r}
#logistic: (doesnt work when i add white and black)
fit <- glm(pred_white ~ median_income + IMR + hispanic + black, data = zipcode, family = "binomial") #not working when i add white and black
score <- predict(fit, type = "response")
class_diag(score, zipcode1$pred_white, positive = 1)


#linear: (this one works)
fit <- lm(pred_white ~ median_income + IMR + hispanic + black, data = zipcode)
score <- predict(fit, type = "response")

table(truth = zipcode$pred_white, predictions = score>.5)
...
knn_fit <- knn3(factor(pred_white==1, levels = c("TRUE", "FALSE")) ~ median_income + IMR + hispanic + black, data = zipcode, k = 5)

y_hat_knn <- predict(knn_fit, zipcode)

table(truth= factor(zipcode$pred_white==1, levels=c("TRUE","FALSE")),
      prediction= factor(y_hat_knn[,1]>.5, levels=c("TRUE","FALSE")))

class_diag(y_hat_knn[,1], zipcode$pred_white, positive = 1)
```
